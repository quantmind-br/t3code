...
export function ProviderIcon(props: ProviderIconProps) {
  ...
  if (props.provider === "muse") {
    // Meta mark, matching `MuseIcon` on the web. Gradient ids are scoped per
    // size so several icons on one screen do not share a `<Defs>` entry.
    const gradientA = `muse-meta-a-${size}`;
    const gradientB = `muse-meta-b-${size}`;
    return (
      <Svg width={size} height={size} viewBox="0 0 290 191" fill="none">
        <Defs>
          <LinearGradient
            id={gradientA}
            x1="61"
